...
/// Switches to the Store tab filtered to [category], reusing the existing
/// shared [ProductsCubit] filter flow (no separate implementation).
///
/// Pass a [GoRouter] captured from a still-valid context (before dismissing any
/// modal) so navigation works even after the caller pops itself.
void openStoreCategory(GoRouter router, ProductCategory category) {
  sl<ProductsCubit>().setCategory(category);
  router.go(AppRoutes.store);
}
